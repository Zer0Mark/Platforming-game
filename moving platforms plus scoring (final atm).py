...
class Player(pygame.sprite.Sprite):  # player object, a sprite is needed to move an object.
    def __init__(self): #initiate self (player)
        super().__init__()
        ...
        self.vel = vec(0, 0) #velocity is set to 0 when player is first spawned
        self.acc = vec(0, 0) #acceleration is also set to 0 when first spawned
        self.jumping = False #make sure it spawns grounded
        self.score = 0 #score is set to 0
    ...
    def move(self):  # movement for the player
        ...
        self.acc.x += self.vel.x * FRIC # self acceleration += self velocity x friction
        self.vel += self.acc # the velocity and acceleration will eventually == the time
        ...
        if self.pos.x > WIDTH:  # screen warping allows the player to gotowards a wall and end up on the other side
            self.pos.x = 0 #spawns player on the left side if they move to the right
        if self.pos.x < 0: #spawns player on the right side if they move to the left
            self.pos.x = WIDTH
        self.rect.midbottom = self.pos  # screen warping

    def jump(self):
        hits = pygame.sprite.spritecollide(self, platforms,
                                           False)  # This gives the ability for the spirtes to collide with each (collison)
        if hits and not self.jumping:
            self.jumping = True
            self.vel.y = -16  # jump height vertical velocity, its negative to be able to give the player jump height

    def cancel_jump(
            self):  # cancel jump is used for short jumps in the game/ or the ability to control what kind of jump you want. since not all platforms will be as high as possilbe.
        if self.jumping:  # if we were to speedrun a platform to regain the ability to jump again then we'd have to short jump to just reach it//make contact with it.
            if self.vel.y < -3:  # this adds a new element of jump to the current one instead of fully jumping to -16, its a lower value so the jump has the ability to short jump
                self.vel.y = -10
            ...
            if hits:  # if hits makes it so there isn't so you can double jump or have multple jumps, so what we want is a single jump
                if self.pos.y < hits[0].rect.bottom:
                    if hits[0].point == True:  ##
                        ...
                        self.score += 1  ##
                    self.pos.y = hits[
                                     0].rect.top + 1  # relocates the player to the top of platform using the variable y-coordinate
...
class platform(pygame.sprite.Sprite):  # Sprites for the platforms/class
    def __init__(self):
        ...
        self.rect = self.surf.get_rect(center=(random.randint(0, WIDTH - 10), random.randint(0,
                                                                                             HEIGHT - 30)))  # puts the platform around the screen in a random location.
